...
means proven by `python -m pytest tests` (168 tests) without the physical Mini.
...
| Endpoints (register/snapshot/heartbeat/delete/display/deck/focus/diagnostics) | `api.py` | `test_api` (REST + SSE, incl. `test_heartbeat_and_delete_via_api`, `test_deck_sse_streams_state_changes`) | headless-verified |
...
```
cd opendeck-broker
python -m pytest tests -q     # 168 pass
