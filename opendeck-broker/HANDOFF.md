...
means proven by `python -m pytest tests` (67 tests) without the physical Mini.
...
| Render identity+state into one image (no stale title) | `images.render_key` | `test` (images), B rows | headless-verified |
...
| Graceful shutdown black-outs keys | `broker.stop` | `test` (stop) | headless-verified |
...
```
cd opendeck-broker
python -m pytest tests -q     # 67 pass
