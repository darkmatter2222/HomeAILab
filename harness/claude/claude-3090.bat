...
REM ============================================================================
REM Claude Code -> DIRECT RTX 3090 llama.cpp (Databrick)
...
REM   Host       : ${HOST_3090}
REM   Port       : 8101  (port contract: 8100-8199 = llama.cpp)
REM   Engine     : llama.cpp server-cuda12
REM   Model alias : qwen3.8
REM
REM NOTE: llama.cpp serves the OpenAI-compatible API (/v1/chat/completions,
REM        /v1/models). It has NO /health endpoint (404), so readiness is
REM        probed by a raw TCP connect (no /health).
REM ============================================================================
...
REM llama.cpp on the 3090 runs without API-key enforcement. Claude Code expects
REM an Anthropic auth-token variable when using ANTHROPIC_BASE_URL, so a local
REM placeholder keeps the client happy.
set "ANTHROPIC_AUTH_TOKEN=local-3090"
...
REM Thinking OFF (matches the 5090's enable_thinking:false contract).
...
set "MODEL_FILE=%TEMP%\claude-3090-model-%RANDOM%-%RANDOM%.tmp"
set "ENDPOINT_FILE=%TEMP%\claude-3090-endpoint-%RANDOM%-%RANDOM%.tmp"

REM ============================================================================
REM AUTO-DETECT DIRECT RTX 3090 ENDPOINT AND DISCOVER THE MODEL
REM ============================================================================
REM Probe likely mappings in preferred order (TCP connect; llama.cpp has no
REM /health, so we probe the published port 8101 directly):
REM   1) ${HOST_3090}:8101  Databrick published port
REM   2) 127.0.0.1:8101       localhost fallback
REM ============================================================================

echo.
echo =============================================================================
echo DIRECT RTX 3090 MODE  ^(direct backend, no router^)
echo =============================================================================
echo   GPU          : RTX 3090
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$candidates = @(" ^
  "  @{ h=$env:HOST_3090; p=8101 }," ^
  "  @{ h='127.0.0.1'; p=8101 }" ^
  ");" ^
  "$selected = $null;" ^
  "foreach ($c in $candidates) {" ^
  "  $url = 'http://' + $c.h + ':' + $c.p;" ^
  "  Write-Host ('  probing ' + $url + ' (TCP) ...') -NoNewline;" ^
  "  try {" ^
  ...
  "if (-not $selected) {" ^
  "  Write-Host '';" ^
  "  Write-Host 'ERROR: No reachable RTX 3090 llama.cpp endpoint was found.' -ForegroundColor Red;" ^
  "  Write-Host ('Checked ' + $env:HOST_3090 + ':8101 and localhost:8101.') -ForegroundColor Yellow;" ^
  "  exit 2;" ^
  "};" ^
...
set "ANTHROPIC_BASE_URL=%QWEN_3090_URL%"
...
REM ============================================================================
REM ROUTE ALL CLAUDE CODE MODEL ROLES TO THE DIRECT RTX 3090 MODEL
REM ============================================================================
...
set "ANTHROPIC_CUSTOM_MODEL_OPTION_NAME=RTX 3090 Direct - %CLAUDE_DGX_MODEL%"
