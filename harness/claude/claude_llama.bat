...
REM ============================================================================
REM Claude Code launcher for local Qwen3.8-27B via llama.cpp
REM
REM Behavior:
...
REM   4. Reads llama.cpp's exact per-slot context length from /slots.
...
REM   7. Leaves speech-to-text disabled in this launcher.
...
REM ============================================================================
REM Local Qwen3.8-27B / llama.cpp configuration
REM ============================================================================

REM Claude Code appends /v1/messages to this origin.
set "ANTHROPIC_BASE_URL=http://127.0.0.1:8000"
set "ANTHROPIC_AUTH_TOKEN=local-llamacpp"

REM Used only if /slots and /v1/models do not expose a usable context length.
set "FALLBACK_CONTEXT_TOKENS=262144"
REM Native Qwen3.8-27B ceiling. The server is launched with 1,048,576 total
REM KV context and --parallel 4, yielding 262,144 tokens per slot.
set "MAX_SESSION_CONTEXT_TOKENS=262144"
...
REM Code's estimate and the Qwen3.8 tokenizer used by llama.cpp can differ.
REM
REM 85%% leaves a ~39.3K token safety margin on a 262,144-token slot.
...
REM ============================================================================
REM Discover available model and exact llama.cpp per-slot context
REM ============================================================================
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "} catch {" ^
  "  Write-Host '';" ^
  "  Write-Host 'ERROR: Could not query the local llama.cpp Qwen3.8-27B endpoint.' -ForegroundColor Red;" ^
  ...
  "$contextSource = '';" ^
  "try {" ^
  "  $slots = @(Invoke-RestMethod -Uri ($env:ANTHROPIC_BASE_URL.TrimEnd('/') + '/slots') -Method Get -Headers $headers -TimeoutSec 10);" ^
  "  $slotContexts = @(" ^
  "    $slots | ForEach-Object {" ^
  "      $candidate = 0L;" ^
  "      if ($null -ne $_.n_ctx -and [long]::TryParse(([string]$_.n_ctx), [ref]$candidate) -and $candidate -gt 0) {" ^
  "        $candidate" ^
  "      }" ^
  "    }" ^
  "  );" ^
  "  if ($slotContexts.Count -gt 0) {" ^
  "    $context = [long](($slotContexts | Measure-Object -Minimum).Minimum);" ^
  "    $contextSource = 'llama.cpp /slots';" ^
  "  }" ^
  "} catch {" ^
  "  Write-Host 'NOTE: /slots did not return a usable per-slot context; checking model metadata/fallback.' -ForegroundColor Yellow;" ^
  "};" ^
  "if ($context -le 0) {" ^
  ...
  "        $context = $candidate;" ^
  "        $contextSource = '/v1/models metadata';" ^
  "        break;" ^
  "      }" ^
  "    }" ^
  "  }" ^
  ...
  "  };" ^
  "  $context = $fallback;" ^
  "  $contextSource = 'configured fallback';" ^
  "};" ^
  "$nativeMax = 0L;" ^
  "if ([long]::TryParse($env:MAX_SESSION_CONTEXT_TOKENS, [ref]$nativeMax) -and $nativeMax -gt 0 -and $context -gt $nativeMax) {" ^
  "  Write-Host ('Reported context ' + $context.ToString('N0') + ' exceeds the configured Qwen3.8 per-session ceiling; capping to ' + $nativeMax.ToString('N0') + '.');" ^
  "  $context = $nativeMax;" ^
  "  $contextSource = $contextSource + ' (capped)';" ^
  "};" ^
  "Write-Host ('Per-session context: ' + $context.ToString('N0') + ' tokens [' + $contextSource + ']');" ^
...
set "ANTHROPIC_CUSTOM_MODEL_OPTION_NAME=Qwen3.8 local - %CLAUDE_DGX_MODEL%"
...
echo   API:            llama.cpp Anthropic Messages API
echo   Slot context:   %CLAUDE_CODE_MAX_CONTEXT_TOKENS% tokens
...
echo   Server tools:   requires llama-server --jinja
