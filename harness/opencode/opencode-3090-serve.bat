...
REM ============================================================================
REM OpenCode -> DIRECT RTX 3090 llama.cpp (Databrick) -- SERVE MODE
REM DIRECT BACKEND (router bypass)
REM
REM FULL 262,144-TOKEN CONTEXT / THINKING OFF / IMAGE INPUT BLOCKED
REM
REM This is the headless-server variant of opencode-3090.bat. It runs
REM `opencode serve --port 4096` so the Stream Deck bridge (and any
REM `opencode attach` client) can discover the session via
REM http://127.0.0.1:4096/global/health.
REM
REM Direct backend:
...
REM ---- OpenCode serve port (Stream Deck bridge probes 4096) ----
set "OPENCODE_SERVE_PORT=4096"
...
set "MODEL_FILE=%TEMP%\opencode-3090-serve-model-%RANDOM%-%RANDOM%.tmp"
set "ENDPOINT_FILE=%TEMP%\opencode-3090-serve-endpoint-%RANDOM%-%RANDOM%.tmp"
set "OPENCODE_CONFIG=%TEMP%\opencode-3090-serve-config-%RANDOM%-%RANDOM%.json"
...
echo.
echo =============================================================================
echo DIRECT RTX 3090 SERVE MODE ^(headless OpenCode server, direct backend^)
echo =============================================================================
echo   GPU          : RTX 3090
...
echo   Serve port   : %OPENCODE_SERVE_PORT% ^(Stream Deck bridge^)
...
echo.
echo Starting OpenCode (headless server)
echo =============================================================================
echo   Mode:           SERVE (headless, Stream Deck bridge discoverable)
echo   Serve URL:      http://127.0.0.1:%OPENCODE_SERVE_PORT%
echo   Health:         http://127.0.0.1:%OPENCODE_SERVE_PORT%/global/health
...
echo.
echo TO ATTACH A TUI TO THIS SERVER (in another terminal):
echo   opencode attach http://127.0.0.1:%OPENCODE_SERVE_PORT%
echo.
...
REM ============================================================================
REM START OPENCODE (headless server)
REM ============================================================================
REM serve runs the headless server; --port pins it to 4096 so the Stream Deck
REM bridge's port scan finds /global/health immediately.
...
echo.
echo OpenCode serve exited with code %EXIT_CODE%.
echo.
